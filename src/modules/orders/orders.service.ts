    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of dto.items) {
        ...
        const product = await tx.product.findFirst({
          where: { id: item.productId, tenantId, active: true, stores: { some: { storeId } } },
          include: { stores: { where: { storeId }, select: { salePrice: true } } },
        });
        ...
        if (available.lt(quantity)) throw new BadRequestException(`Estoque insuficiente para ${product.name}.`);

        const reservation = await tx.stockReservation.create({
          data: { tenantId, storeId, productId: product.id, quantity, reference: 'ORDER_PENDING', expiresAt, createdBy: actorId },
        });
      ...
      const order = await tx.order.create({
        data: {
          tenantId, storeId, origin: dto.origin, type: dto.type ?? 'PICKUP', status: OrderStatus.PENDING_PAYMENT,
          customerName: dto.customerName?.trim(), customerPhone: dto.customerPhone?.trim(), notes: dto.notes?.trim(),
          subtotal, totalAmount: subtotal, expiresAt, createdBy: actorId,
          items: { create: lines },
        }, include: { items: true },
      });
